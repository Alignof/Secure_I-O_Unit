...
long int touch1(int pulse,int sensor){
	...
	//Serial.printf("Left sensor:%d t:%d\n",sensor,t);
...
long int touch2(int pulse,int sensor){
	...
	//Serial.printf("Right sensor:%d t:%d\n",sensor,t);
	return t;
}
...
void Right_hand(void *pvParameters){
	...
	while(1){
		...
		if(out!=-1 && (millis()-prev)>((out==buf)?20:50)){
...
void setup() {
	...
	xTaskCreatePinnedToCore(Right_hand, "Right_hand", 4096, NULL, 2, NULL, 0);
	xTaskCreatePinnedToCore(Left_hand, "Left_hand", 4096, NULL, 2, NULL, 0);
}
